...
//sort results
if ( is_string($request->order) && is_bool($request->ascending) )
	usort($response->rows, $sortFunction($request->order, $request->ascending));

$response->rows = extractPageFromArray($response->rows, intval($request->page), intval($request->pagesize) );
